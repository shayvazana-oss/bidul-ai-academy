...
Before firing any generation, calculate and present total credit cost to the user. Standard KIE rule (see [KIE SKILL.md "Credit cost estimation"](../../arcads-external-api/SKILL.md#credit-cost-estimation-mandatory--show-before-generating)).
...
Reference image delivery requires a public URL (CDN/bucket/Cloudinary/etc.). KIE has no presigned-upload flow. Auto-upscale any image below 1024 px longest side per [KIE SKILL.md "Image handling"](../../arcads-external-api/SKILL.md#image-handling-auto-upscale-small-inputs).
